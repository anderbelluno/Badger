...
procedure TSyUtils.GetMIMETableFromOS(const AMIMEList: TStringList);
...
procedure TSyUtils.BuildMimelist(aMimeList : TStringList);
begin
...
destructor TSyUtils.Destroy;
begin
  FMimeList.Free;
  inherited;
end;

function TSyUtils.ExtractMethodAndURI(const RequestLine: string;
...
function TSyUtils.GetFileMIMEType(const AFileName: string): string;
var
  Index: integer;
  LExt: string;
begin
