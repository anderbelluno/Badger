...
type
 ...
 private
  ...
  // procedure SaveStreamToDataSet(aStream : TMemoryStream);
...
procedure TSynaMethods.AtuImage(ClientSocket: TTCPBlockSocket; const URI,
   ...
   try
     ...
     for i := 0 to Files.Count - 1 do
      begin
        ...
        //ShowMessage(Format('Arquivo salvo: %s, Extensão: %s, Tamanho: %d bytes', [UniqueName, FormDataFile.Extension, FormDataFile.Stream.Size]));
      end;

      if V_ErrMsg = EmptyStr then
         ClientSocket.SendString('HTTP/1.1 200 OK' + CRLF + 'Content-Type: text/plain' + CRLF + CRLF )
      else
         ClientSocket.SendString( 'HTTP/1.1 500 ' + UTF8Encode(V_ErrMsg) + CRLF + 'Content-Type: text/plain' + CRLF + CRLF );  //Ok Funciona
...
procedure TSynaMethods.fDownloadStream(
  const ClientSocket: TTCPBlockSocket; const URI, Method,
  RequestLine: String);
var
  Header: string;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        //SaveStreamToDataSet(BodyStream);
...
{procedure TSynaMethods.SaveStreamToDataSet(aStream: TMemoryStream);
var
 aDataSet : TVirtualTable;
begin
  aDataSet := nil;
  aDataSet:= TVirtualTable.Create(nil);
  aDataSet.Open;

  try
    aDataSet.LoadFromStream(aStream);
    TBlobField(aDataSet.FieldByName('img')).SaveToFile('.\img.jpg');
  finally
    aDataSet.Free;
  end;
end;  }
