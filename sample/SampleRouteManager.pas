...
begin
  ...
  begin
    ...
    Response.ContentType := APPLICATION_JSON;
    Response.Body := '{"error":"Credenciais inv�lidas"}';
  end;
end;
...
begin
  ...
  if Request.RouteParams.Count > 0 then
    Response.Body := Format('id: %s' + sLineBreak +  'c�digo: %s', [Request.RouteParams.Values['id'], Request.RouteParams.Values['codigo'] ])
  else
    Response.Body := Format('id: %s' + sLineBreak +  'c�digo: %s', [Request.QueryParams.Values['id'], Request.QueryParams.Values['codigo'] ]);
end;
...
begin
  ...
  begin
    ...
    Response.Body := '{"error":"Refresh token n�o fornecido"}';
    Response.ContentType := APPLICATION_JSON;
    Exit;
  end;
